#line 1 "/mnt/6E9A84429A8408B3/_VSC/WEB_UPLOAD/HELTEC_ESPNOW_TREE_BCAST/DataManager.cpp"
...
void DataManager::processDistributedIOUpdate(const TreeMessageHeader* header, const uint8_t* payload, size_t payloadLen, const uint8_t* sender) {
    ...
    if (payloadLen != sizeof(DistributedIOData)) {
        dataLog("CHILD: Invalid distributed I/O update size: " + String(payloadLen), 1);
        return;
    }
    ...
    const DistributedIOData* data = (const DistributedIOData*)payload;
    ...
    dataLog("CHILD: Device " + formatHID(systemStatus.myHID) + 
           ...
           "SharedData:" + formatDistributedIOData(*data), 2);
    
    // Get old shared data before updating
    uint32_t oldSharedData = getDistributedIOSharedData().words[0];
    
    // Update both DataManager's state (for display) and IoDevice's state (for outputs)
    setDistributedIOSharedData(*data);
    IO_DEVICE.processSharedDataUpdate(*data);
    
    // Log button press/release events to console
    uint32_t newSharedData = data->words[0];
    consoleLogSharedDataChange(oldSharedData, newSharedData);

    dataLog("CHILD: Processed distributed I/O update: " + formatDistributedIOData(*data), 2);
    
    // Forward to my direct children (tree-based distribution)
    forwardDistributedIOUpdateToChildren(*data);
}
...
void DataManager::computeAndBroadcastDistributedIO() {
    ...
    for (int i = 0; i < SHARED_DATA_WORDS; i++) {
        if (newSharedData.words[i] != currentSharedData.words[i]) {
            dataChanged = true;
            break;
        }
    }
    ...
    if (dataChanged) {
        ...
        // Log button press/release events to console
        consoleLogSharedDataChange(currentSharedData.words[0], newSharedData.words[0]);
...
DistributedIOData DataManager::computeSharedDataFromInputs() const {
    ...
    if (isDeviceFullyConfigured()) {
        ...
        // Check if the first input pin is active (pressed).
        // The readInputPins() function in IoDevice sets bit 0 to '1' if the pin is LOW.
        if ((myData.input_states & 0x01) != 0) { 
            ...
            if (isValidBitIndex(myBitIndex)) {
                ...
                sharedData.words[wordIndex] |= (1UL << bitInWord);
                dataLog("Root input active -> setting bit " + String(myBitIndex) + 
                       " in shared data", 2);
            } else {
                dataLog("ERROR: Root has invalid bit index: " + String(myBitIndex), 1);
            }
        } else {
            dataLog("Root input not active (input_states & 0x01 = 0)", 4);
        }
    } else {
    ...
    for (int i = 0; i < aggregatedDeviceCount; i++) {
        ...
        // Check if the remote device's first input pin is active.
        if ((deviceData.input_states & 0x01) != 0) {
            ...
            if (isValidBitIndex(deviceBitIndex)) {
                ...
                dataLog("Device " + formatHID(deviceHIDArray[i]) + 
                       " (bit " + String(deviceBitIndex) + ") input active -> setting its bit", 4);
            } else {
...
String DataManager::formatDistributedIOData(const DistributedIOData& data) const {
    ...
    for (int i = 0; i < SHARED_DATA_WORDS; i++) {
        if (i > 0) result += " ";
        char wordStr[12];
        snprintf(wordStr, sizeof(wordStr), "0x%08lX", data.words[i]);
...
uint16_t DataManager::getDistributedIOSharedDataAsUint16() const {
    ...
    return (uint16_t)(data.words[0] & 0xFFFF);
}
...
void DataManager::setDistributedIOBit(int bitIndex, bool value) {
    ...
    if (value) {
        data.words[wordIndex] |= (1UL << bitInWord);
    } else {
        data.words[wordIndex] &= ~(1UL << bitInWord);
    }
...
bool DataManager::getDistributedIOBit(int bitIndex) const {
    ...
    return (distributedIOData.words[wordIndex] & (1UL << bitPosition)) != 0;
}
...
uint32_t DataManager::getSharedData() const {
    // Return the first word of distributed I/O data as uint32
    return distributedIOData.words[0];
}

bool DataManager::getMyBitState() const {
    // Get the state of this device's assigned bit
    if (systemStatus.bitIndexConfigured) {
        return getDistributedIOBit(systemStatus.myBitIndex);
    }
    return false;
}
