...
void IoDevice::updateOutputsFromSharedData(const DistributedIOData& sharedData) {
    // This maps the 32-bit shared data to local outputs to show the state of all devices
    uint8_t outputStates = sharedData.words[0] & 0xFF;
    
    ioLog("Updating outputs from shared data - old outputs: " + String(currentOutputStates, BIN) + 
         " new outputs: " + String(outputStates, BIN) + 
         " (shared: " + DATA_MGR.formatDistributedIOData(sharedData) + ")", 2);
    
    updateOutputs(outputStates);
    
    ioLog("Outputs updated from shared data: " + String(outputStates, BIN) + 
